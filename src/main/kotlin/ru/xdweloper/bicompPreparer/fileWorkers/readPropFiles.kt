...
data class PropFile(val readDir:String = "",
    ...
    fun checkProps(): Pair<ResaltState,String>{
        ...
        if(!File(buildPath(sourceDir,changeFile)).isFile  || !File(buildPath(sourceDir,changeFile)).exists())
            return Pair(ResaltState.ERROR,"$_CHECK_PROP_DATA_FILE changeFile")
